...
status_val ef_tree_get(struct ef_tree *root, const char *tag, struct ext_filter **e)
{
    ...
    if (!root->flt && !strcmp(tag, root->flt->filter->packet_tag)) { //checking if current node is correct one
        *e = root->flt;
        return STATUS_OK;
    }
...
void ef_tree_free(struct ef_tree *root)
{
    ...
    while (cur) {
        next = cur->next; //cur will be freed in next step, so I have to get next node now
        ef_tree_free(cur);
        cur = next;
    }
