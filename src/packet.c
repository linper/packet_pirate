...
static status_val packet_get_entry(struct packet *p, const char *tag, struct p_entry **e)
{
...
static status_val get_entry_length(struct glist *pkt_list, struct ef_tree *node, struct f_entry *fe, struct p_entry *e) //TODO finish 
{
	...
	glist_foreach(p, pkt_list) {
	    if (!packet_get_entry((struct packet*)p, fe->tag, &pe) && \
		    bytes_to_uint(pe->raw_data, pe->raw_len, (unsigned long*)&e->raw_len)) {
...
static status_val derive_entry(struct glist *pkt_list, struct ef_tree *node, struct packet *p, struct f_entry *fe, struct p_entry *e, const u_char *data, unsigned len, unsigned *read_off) //TODO finish
{
    status_val status;

    e->tag = fe->tag;
    if (get_entry_length(pkt_list, node, fe, e)) { //getting data length of current entry
	free(e);
...
status_val derive_packet(struct glist *pkt_list, struct ef_tree *node, const u_char *data, unsigned len, unsigned *read_off)
{
	...
	status = derive_entry(pkt_list, node, p, &node->flt->filter->entries[i], &p->entries[i], data, len, read_off);
	if (status) {
